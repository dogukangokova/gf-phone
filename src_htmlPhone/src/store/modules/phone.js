...
const getters = {
  ...
  BottomApps: ({ config, lang }, getters) => config.bottomapps
    .filter(app => app.enabled !== false)
    .map(app => {
      if (app.puceRef !== undefined) {
        app.puce = getters[app.puceRef]
  ...
  AppStoreApps: ({ config, lang }, getters) => config.appstoreapps
    .filter(app => app.enabled !== false)
    .map(app => {
      if (app.puceRef !== undefined) {
        app.puce = getters[app.puceRef]
...
const mutations = {
  ...
  SET_APP_ENABLE (state, {appName, enable}) {
    ...
    const appIndexBottom = state.config.bottomapps.findIndex(app => app.name === appName)
    if (appIndexBottom !== -1) {
      Vue.set(state.config.bottomapps[appIndex], 'enabled', enable)
    }

    const appIndexAppStoreApps = state.config.appstoreapps.findIndex(app => app.name === appName)
    if (appIndexAppStoreApps !== -1) {
      Vue.set(state.config.appstoreapps[appIndex], 'enabled', enable)
    }
  },
